...
async function createTables() {
    try {
        ...
        const tableCount = existingTables[0].count || existingTables[0]['count(*)'];

        if (parseInt(tableCount, 10) > 0) {
            log.info("mysql", `⏭️ Skipping table creation: ${tableCount} table(s) already exist in database '${dbName}'`);
            return "skipped";
        }

        log.info("mysql", "✅ No existing tables found. Creating tables...");
        ...
        // docker_host
        const hasDockerHost = await knex.schema.hasTable("docker_host");
        if (!hasDockerHost) {
        ...
        // group
        const hasGroup = await knex.schema.hasTable("group");
        if (!hasGroup) {
        ...
        // proxy
        const hasProxy = await knex.schema.hasTable("proxy");
        if (!hasProxy) {
        ...
        // heartbeat
        const hasHeartbeat = await knex.schema.hasTable("heartbeat");
        if (!hasHeartbeat) {
        ...
        // incident
        const hasIncident = await knex.schema.hasTable("incident");
        if (!hasIncident) {
        ...
        // maintenance
        const hasMaintenance = await knex.schema.hasTable("maintenance");
        if (!hasMaintenance) {
...
// Final invocation and log control
createTables()
    .then((status) => {
        if (status === "skipped") {
            log.info("mysql", "ℹ️ Table creation skipped — already initialized.");
        } else if (status === "created") {
            log.info("mysql", "✅ MySQL schema creation complete.");
        }
    })
    .catch((err) => {
        log.error("mysql", "❌ Error in schema creation: " + err);
    })
    .finally(() => {
        knex.destroy();
    });
